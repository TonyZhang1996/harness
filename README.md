...
- 侧边栏可折叠为细轨并持久化状态（重启后保留折叠/展开）。
- 修正权限模式文档，明确 `never` 模式不通过 CLI 暴露。

## 当前边界

AI Harness 0.6.2 是一个支持 Windows、macOS 和 Linux 的本地编码 Agent MVP，并提供 Tkinter 桌面 GUI。真正的二进制办公文件生成、上下文压缩、沙箱容器和分布式执行属于后续版本范围。
